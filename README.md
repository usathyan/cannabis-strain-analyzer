...
Scripts in the `android-app/scripts/` directory manage the embedded strain database:

```bash
# List configured strains
python3 android-app/scripts/build_strain_database.py --list

# Add a new strain to config
python3 android-app/scripts/build_strain_database.py --add-strain "Strain Name"

# Rebuild database from API + fallbacks
python3 android-app/scripts/build_strain_database.py
```

Configuration: `android-app/scripts/strain_config.json`
- `strains`: List of strain names to include
- `fallback_strains`: Curated data for strains not in API

Output: `android-app/app/src/main/assets/strains.json`
...
Curious about the development process? Read **[BUILDING.md](android-app/BUILDING.md)** — a chronicle of human-AI collaboration that took this project from a Python web app to a privacy-first Android application with on-device AI.
